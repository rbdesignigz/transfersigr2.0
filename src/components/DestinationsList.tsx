...
export default function DestinationsList({ destinations, onSelectDestination, loading = false }: DestinationsListProps) {
  ...
  return (
    <section className="py-16 px-6 max-w-7xl mx-auto relative">
      ...
      <div className="mb-12 flex flex-col items-center text-center border-b border-gray-800 pb-8">
        <div className="text-[#65d6e9] font-mono text-[10px] tracking-[0.2em] uppercase mb-2 flex items-center gap-2">
          EXPLORE LA TRIPLE FRONTERA
        </div>
        <h2 className="text-3xl md:text-5xl font-display font-extrabold text-white mb-4 tracking-tighter uppercase">
          DESTINOS
        </h2>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4">
          <button
            ...
            className={`px-5 py-2 border font-mono text-[10px] tracking-widest uppercase transition-all flex items-center gap-2 cursor-pointer ${
              filterCode === 'ALL'
                ? 'bg-[#65d6e9]/10 text-[#65d6e9] border-[#65d6e9] shadow-[0_0_15px_rgba(101,214,233,0.3)]'
                : 'border-gray-800 text-gray-400 hover:text-[#65d6e9] hover:border-[#65d6e9]/40'
            }`}
          >
            <span className={`w-1 h-1 rounded-full ${filterCode === 'ALL' ? 'bg-[#65d6e9]' : 'bg-gray-500'}`}></span>
            Todos los países
          </button>
          ...
          {(['AR', 'BR', 'PY'] as const).map(code => {
            ...
            return (
              ...
              >
                <span className={`w-1 h-1 rounded-full ${isActive ? 'bg-[#65d6e9]' : 'bg-gray-500'}`}></span>
                {label}
              </button>
            );
          })}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[1, 2, 3].map((n) => (
            <div key={n} className="bg-[#0f1214] rounded-xl h-[480px] border border-gray-900 animate-pulse flex flex-col overflow-hidden">
              <div className="h-64 bg-gray-800/30"></div>
              <div className="p-6 space-y-4 flex-grow">
                <div className="h-6 bg-gray-800/30 w-2/3 rounded"></div>
                <div className="space-y-2">
                  <div className="h-3 bg-gray-800/20 rounded"></div>
                  <div className="h-3 bg-gray-800/20 rounded"></div>
                  <div className="h-3 bg-gray-800/20 rounded w-5/6"></div>
                </div>
                <div className="pt-6 border-t border-gray-900 space-y-3 mt-auto">
                  <div className="h-8 bg-gray-800/20 rounded"></div>
                  <div className="h-8 bg-gray-800/20 rounded"></div>
                </div>
              </div>
            </div>
        ...
        <div className="text-center py-16 border border-gray-800 bg-[#0f1214] rounded-xl font-mono text-xs uppercase tracking-widest text-gray-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          >
            {/* Image section */}
              ...
              <div className="absolute top-4 right-4 bg-[#0a0c0d]/80 backdrop-blur-md text-white font-mono text-[9px] px-2.5 py-1 border border-white/20 rounded font-bold uppercase tracking-widest">
                {dest.code}
              </div>

              {/* Gradient dark frame mask */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#0f1214] to-transparent opacity-60"></div>
            </div>
